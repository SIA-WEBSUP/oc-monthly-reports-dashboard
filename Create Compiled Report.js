...
function appendEndOfReportsSection_(body, notSubmitted, skipped) {
  ...
  if (notSubmitted.length) {
    ...
    for (let i = 0; i < notSubmitted.length; i += COLS) {
      const rowItems = notSubmitted.slice(i, i + COLS);
      const line = rowItems
        .map(label => ('- ' + label).padEnd(COL_WIDTH))
        .join('')
        .trimEnd();
  ...
  if (skipped.length) {
    ...
    for (let i = 0; i < skipped.length; i += COLS) {
      const rowItems = skipped.slice(i, i + COLS);
      const line = rowItems
        .map(label => ('- ' + label).padEnd(COL_WIDTH))
        .join('')
        .trimEnd();
